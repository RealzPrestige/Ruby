...
public class GuiFloat extends GuiSetting {
    FloatSetting setting;
    int hoverAnimWidth;

    public GuiFloat(FloatSetting setting) {
        super(setting);
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        Ruby.rubyFont.drawStringWithShadow(roundNumber(setting.getValue(), 1) + "", x + width - Ruby.rubyFont.getStringWidth(setting.getValue() + "") - 1, y + (height / 2f) - (Ruby.rubyFont.getHeight(setting.getName() + " " + setting.getValue()) / 2f), Color.GRAY.getRGB());
    ...
    public float sliderWidthValue() {
        return (((Number) setting.getValue()).floatValue() - setting.getMinimum()) / (setting.getMaximum() - setting.getMinimum());
    }
